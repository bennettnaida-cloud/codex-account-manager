...
/// <summary>
/// Persists the gateway's latest HTTP 429 observations without ever storing a token,
...
internal sealed class PatGatewayQuotaSignalStore
{
        ...
        DateTimeOffset observedAtUtc)
    {
        ...
        lock (_gate)
        {
            ...
            var signal = new PatGatewayQuotaSignal(nextSequence, normalizedKey, observed);
    ...
    internal static void Validate()
    {
        ...
        try
        {
            ...
            var first = store.Record(firstKey, now.AddSeconds(-10));
            ...
            {
                throw new InvalidOperationException(
                    "A signal must remain available long enough to be matched to its official five-hour window after Manager restart.");
            }
