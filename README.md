...
### 3. 官方 Codex 的 Standard/Fast 兼容层

PAT 或 API Key 账号由管理器启动官方 Codex 时，程序会在经过进程身份、回环端口和页面目标三重验证后，通过 CDP 对当前内存中的 renderer 做版本锁定的兼容处理；不会修改已签名的 MSIX 或 ASAR 文件。若 Codex 更新后代码契约不再匹配，兼容层会拒绝修改并记录本机诊断，等待管理器更新。

此功能是非官方的本地界面兼容层。Fast 仍对应 Codex 使用的 `priority` 服务层级；实际端点是否支持、是否可用以及如何计费，始终由服务端或第三方兼容 API 决定，看到按钮不代表获得了额外权限。
